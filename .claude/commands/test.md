...
- **Content extremes, for anything that renders copy.** The shortest string the schema allows, a typical
  ...
  no layout, so a unit test can pin a class string and no more: the measurement is
  `npm run smoke:dev` for the dev-only fixture (`/dev/ui/content-stress` at 390 / 640 / 768 / 1024 /
  1440) — the CI `dev-smoke` job runs it on every PR, so run it locally only to measure. Add a case to
