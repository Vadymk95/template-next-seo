...
describe('gate preflight', () => {
    ...
    it('--kill-port clears a stray listener and proceeds, naming the pid it killed', async () => {
        ...
        const holder = spawn(
            ...
            [
                '-e',
                'require("net").createServer().on("error", () => process.exit(2)).listen(3181, () => {}); setInterval(() => {}, 1000)'
            ],
            { stdio: 'ignore' }
        );
        ...
        await new Promise((resolvePromise, reject) => {
            const probe = () => {
                if (holder.exitCode !== null) {
                    reject(
                        new Error(
                            `holder exited with ${String(holder.exitCode)} before listening on 3181`
                        )
                    );
                    return;
                }
                let connected = false;
                const socket = connect(3181, () => {
                    connected = true;
                    socket.destroy();
                    resolvePromise(null);
                });
